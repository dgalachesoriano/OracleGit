...
def test_create_snapshot_calls_dependencies_correctly():
    ...
    service = SnapshotService(
        db_connector=db_connector_mock, snapshot_repository=snapshot_repo_mock
    )
...
def test_list_snapshots_delegates_to_repository():
    snapshot_repo_mock = Mock()
    snapshot_repo_mock.list_snapshots.return_value = [
        "snapshots/config_servicio/20240614T102000Z.json"
    ]

    service = SnapshotService(
        db_connector=Mock(), snapshot_repository=snapshot_repo_mock
    )
...
def test_load_latest_snapshot_delegates_to_repository():
    ...
    fake_snapshot = ParamSnapshot(
        table_name="config_servicio", timestamp=datetime.now(UTC), entries=[]
    )
